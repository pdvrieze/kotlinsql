...
abstract class Database constructor(@Suppress("MemberVisibilityCanBePrivate") val _version: Int) : DatabaseMethods() {
    ...
    interface Insert : UpdatingStatement {
        ...
        fun <T : Any, R> executeSeq(connection: DBConnection2<*>, key: Column<T, *, *>, autogeneratedKeys: (T?) -> R): Sequence<R>
        fun <T : Any> executeSeq(connection: DBConnection2<*>, key: Column<T, *, *>): Sequence<T>
    ...
    abstract class _BaseInsert(val table: Table, private val update: Boolean, vararg val columns: ColumnRef<*, *, *>) : Insert {
         ...
         * @see java.sql.PreparedStatement.executeUpdate
         */
        override fun <T : Any, R> executeSeq(
            connection: DBConnection2<*>,
            key: Column<T, *, *>,
         ...
         * @see java.sql.PreparedStatement.executeUpdate
         */
        override fun <T : Any> executeSeq(connection: DBConnection2<*>, key: Column<T, *, *>): Sequence<T> {
            if (batch.isEmpty()) {
                throw IllegalStateException("There are no values to add")
            }
            return connection.prepareStatement(toSQL()) {
...
fun <DB: Database, T, R: Any> DBTransactionBase<DB, T>.insert(keyId: Column<R,*, *>, stmtGen: DB.(T)-> Database.Insert): DBTransaction<DB, Sequence<R>> {
    return map {
        db.stmtGen(it).executeSeq(connection, keyId)
    }
}

fun <DB: Database, T, K: Any, R> DBTransactionBase<DB, T>.insert(
    keyId: Column<K, *, *>,
    autogeneratedKeys: (K?) -> R,
    stmtGen: DB.(T) -> Database.Insert
                                                             ): DBTransaction<DB, Sequence<R>> {
