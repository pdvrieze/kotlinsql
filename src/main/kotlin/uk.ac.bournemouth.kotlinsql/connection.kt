...
class DBConnection constructor(private val connection: Connection, val db: Database?=null) {
  ...
  fun <R> prepareStatement(sql: String, resultSetType: Int,
    ...
    return connection.prepareStatement(sql, resultSetType, resultSetConcurrency).use { block(StatementHelper(it, sql)) }
  }
  ...
  fun <R> prepareStatement(sql: String, resultSetType: Int,
    ...
    return connection.prepareStatement(sql, resultSetType, resultSetConcurrency, resultSetHoldability).use { block(StatementHelper(it, sql)) }
  }

  @Throws(SQLException::class)
  fun <R> prepareStatement(sql: String, autoGeneratedKeys: Int, block: (StatementHelper) -> R): R {
    return connection.prepareStatement(sql, autoGeneratedKeys).use { block(StatementHelper(it, sql)) }
  }

  @Throws(SQLException::class)
  fun <R> prepareStatement(sql: String, columnIndexes: IntArray, block: (StatementHelper) -> R): R {
    return connection.prepareStatement(sql, columnIndexes).use { block(StatementHelper(it, sql)) }
  }

  @Throws(SQLException::class)
  fun <R> prepareStatement(sql: String, columnNames: Array<out String>, block: (StatementHelper) -> R): R {
    return connection.prepareStatement(sql, columnNames).use { block(StatementHelper(it, sql)) }
  }
