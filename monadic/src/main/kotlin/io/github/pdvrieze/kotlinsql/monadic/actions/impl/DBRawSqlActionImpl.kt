 ...
 */
internal class DBRawSqlActionImpl<DB : Database, out O>(
    val sql: String,
    val action: (PreparedStatementHelper) -> O
) : DBBaseSqlAction<DB, O>() {

    override fun getQuery(): String = sql

    override fun doEval(connection: MonadicDBConnection<DB>, initResult: PreparedStatementHelperImpl): O {
        return action(initResult)
    }
}