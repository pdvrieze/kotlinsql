...
import uk.ac.bournemouth.kotlinsql.monadic.impl.ResultSetIterator
import uk.ac.bournemouth.kotlinsql.monads.impl.gen.ConnectionSource
import uk.ac.bournemouth.kotlinsql.queries.*
import uk.ac.bournemouth.kotlinsql.queries.impl._Where
...
sealed class DBAction<DB : Database, out O> {
    ...
    fun <R> flatMap(actionProducer: (O) -> Iterable<DBAction<DB, R>>): DBAction<DB, List<R>> = GroupAction(this, actionProducer)
...
//        inline fun <T : Connection, R> T.use(block: (T) -> R) = useHelper({ it.close() }, block)
        ...
        private inline fun <DB : Database, R> DataSource.withConnection(db: DB, block: (MonadicDBConnection<DB>) -> R): R {
...
@DbActionDSL
class SelectAction<DB : Database, S : SelectStatement>(val query: S) : DBAction<DB, ResultSet>() {
...
class GroupAction<DB: Database, I, out O>(val before: DBAction<DB, I>, val actionProducer: (I) -> Iterable<DBAction<DB, O>>): DBAction<DB, List<O>>() {
