...
import uk.ac.bournemouth.kotlinsql.metadata.ColumnsResults
...
import uk.ac.bournemouth.kotlinsql.monads.impl.gen.DBActionReceiver
import java.sql.SQLException
import java.util.*
import uk.ac.bournemouth.kotlinsql.monads.impl.gen.invoke as generatedInvoke
...
internal abstract class ConnectionSourceImplBase<DB : Database>: ConnectionSource<DB> {
    ...
    override fun Table.createTransitive(
        ...
        val neededTables = (_cols.asSequence().mapNotNull { col -> col.references?.table } +
                _foreignKeys.asSequence().mapNotNull(ForeignKey::toTable))
            .map { db[it._name] }
